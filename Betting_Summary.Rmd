...
```{r}
betting_summary <- sqldf("select distinct season_type
              , season
              , week
              , game_date
...
#write.csv(betting_summary, "C:\\Users\\mikey\\OneDrive\\Documents\\Madden\\Betting_Summary.csv", row.names=FALSE)

```

# Team Stats trying to create team stats for Defense and Offense while also using fantasy Points to create rankings for each team. I realize now that I should create these on a play by play level so that I can roll them up to any level possible.
...
#view(sqldf("select * from fd_df where posteam is NULL;"))
